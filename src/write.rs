...
impl<'a, W: io::Write + io::Seek> ExtraDataWriter<'a, W> {
    pub fn original_file_start(&self) -> u64 {
    ...
    pub fn write_central_extra_data(self) -> io::Result<CentralExtraDataWriter<'a, W>> {
        validate_extra_data(&self.data)?;
...
impl<W: io::Write + io::Seek> ZipWriter<W> {
    ...
    fn start_entry(
        ...
        write_local_file_header(&mut self.writer, &file)?;
        ...
        S: Into<String>,
    {
        ...
        let data_start = writer.original_file_start();
    ...
    /// use zip::{ZipArchive, ZipWriter};
    ///
    /// fn copy<R, W>(src: &mut ZipArchive<R>, dst: &mut ZipWriter<W>) -> std::io::Result<()>
    /// where
    ///     R: Read + Seek,
    ///     W: io::Write + Seek,
    /// {
    ...
    fn finalize(&mut self) -> io::Result<()> {
        ...
        for file in &self.files {
            write_central_directory_header(&mut self.writer, file)?;
        }
        let central_size = self.writer.stream_position()? - central_start;
            ...
            || central_size.max(central_start) > spec::ZIP64_BYTES_THR
        {
            ...
            zip64_footer.write(&mut self.writer)?;
