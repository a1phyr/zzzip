...
pub(crate) mod zip_archive {
    ...
    /// ```no_run
    /// use std::io::prelude::*;
    /// fn list_zip_contents(reader: impl BufRead + Seek) -> zip::result::ZipResult<()> {
    ///     let mut zip = zip::ZipArchive::new(reader)?;
    ///
    ///     for i in 0..zip.len() {
    ///         let file = zip.by_index(i)?;
    ///         println!("Filename: {}", file.name());
    ///         std::io::copy(&mut file.reader()?, &mut std::io::stdout());
    ///     }
    ///
    ///     Ok(())
    /// }
...
impl<R: io::BufRead + io::Seek> ZipArchive<R> {
    ...
    pub fn by_index(&mut self, file_number: usize) -> ZipResult<ZipFile<'_, R>> {
        ...
        find_content(&mut self.reader, &data.data)?;
    ...
    R: io::BufRead + io::Seek,
{
    ...
    pub fn decrypt(mut self, password: &[u8]) -> ZipResult<EncryptedZipFileReader<'a>> {
        let meta = self.data;
    ...
    pub fn raw_reader(mut self) -> RawZipFileReader<'a> {
        RawZipFileReader {
